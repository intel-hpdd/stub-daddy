...
describe('entry', function () {
  var entry, logger, mockStatus, requestMatcher;
  ...
  describe('request entry', function () {
    var request, response, dependencies, requestEntry, entries;
    beforeEach(function () {
      request = {
        method: 'GET',
        url: '/api/dest',
    ...
    describe('adding an entry', function () {
      beforeEach(function () {
        entry.addEntry(requestEntry.request, requestEntry.response, requestEntry.expires,
          requestEntry.dependencies, requestEntry.timeout, entries);
      });

      it('should call logByLevel', function () {
        expect(logger.logByLevel).toHaveBeenCalledOnceWith({
          DEBUG: [{url: requestEntry.request.url}, 'adding entry to request store.'],
          TRACE: [{
    ...
    describe('find entries', function () {
      ...
      describe('with filtered entries', function () {
        beforeEach(function () {
          ...
          selectedEntries = entry.findEntriesByRequest(mockStatus, requestEntry.request, entries);
        });

        it('should call requestMatcher', function () {
          expect(requestMatcher).toHaveBeenCalledOnceWith(requestEntry.request, entries[0].request);
        });

        it('should call mockStatus.haveRequestsBeenSatisfied', function () {
          expect(mockStatus.haveRequestsBeenSatisfied).toHaveBeenCalledOnceWith(requestEntry.dependencies);
        });
      ...
      describe('without filtered entries', function () {
        ...
        beforeEach(function () {
          ...
          selectedEntries = entry.findEntriesByRequest(mockStatus, request, entries);
        });

        it('should call logger.warn', function () {
          expect(logger.warn).toHaveBeenCalledOnceWith({request: request}, 'entry for request not found');
        });
        ...
        it('should not call mockStatus.haveRequestsBeenSatisfied', function () {
          expect(mockStatus.haveRequestsBeenSatisfied).not.toHaveBeenCalledOnce();
        });
    ...
    describe('updating an entry', function () {
      var filesystemEntry, filterFilesystemEntry, entry1, entry2;
      beforeEach(function () {
        ...
        entry1 =  {
          request: fixtures.integration.registerRequestWithDependencies.json.json.request,
          response: fixtures.integration.registerRequestWithDependencies.json.json.response,
          expires: fixtures.integration.registerRequestWithDependencies.json.json.expires,
          dependencies: [obj.merge({}, fixtures.integration.registerRequestWithDependencies.json.json.dependencies[0],
            {url: '/api/filesystem'})],
          timeout: 0,
          calls: 0,
          remainingCalls: 1
        ...
        filterFilesystemEntry = fp.flow(fp.filter(fp.flow(fp.pathLens(['request', 'url']), fp.eq('/api/filesystem'))));
        filesystemEntry = fp.filter(filterFilesystemEntry, entries)
          .pop();
    ...
    describe('flushing entries', function () {
      ...
      it('should log', function () {
        expect(logger.info).toHaveBeenCalledOnceWith('flushing request store entries');
      });
    });
  });
  ...
  describe('single request', function () {
    var request, response, dependencies, entries, requestEntry;
    beforeEach(function () {
      entries = [];
    ...
    describe('matching', function () {
      ...
      it('should call requestMatcher with request and entry.request', function () {
        expect(requestMatcher).toHaveBeenCalledOnceWith(request, foundEntry.request);
      });
    });

    describe('multiple requests matching', function () {
      var request2, response2, dependencies2;
      ...
      describe('some of which have their dependencies met', function () {
        var request3, response3, foundEntries;
        beforeEach(function () {
          ...
          mockStatus.haveRequestsBeenSatisfied.and.callFake(function (dependencies) {
